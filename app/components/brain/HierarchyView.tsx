...
import type { Entity, Fund, Sponsor, DirectInvestment } from "~/lib/types";
import { formatCurrency, formatMultiplier, formatIrr } from "~/lib/utils";
import { useT } from "~/lib/use-t";

/**
 * "Jerarquía" view — the original top-down SVG org tree. Extracted verbatim
 * from the Entity Map index route so it can sit behind the Brain/Hierarchy
 * toggle. Behaviour (expand/collapse, shift-click reassign, tooltips, entity
 * pills, empty state) is unchanged; it now receives loader data via props
 * instead of reading it itself.
 */
export function HierarchyView({
  data,
  orgName,
}: {
  data: { entities: Entity[]; funds: Fund[]; sponsors: Sponsor[] };
  orgName: string;
}) {
  ...
  const reassignFetcher = useFetcher<{ intent?: string; ok?: boolean; error?: string }>();
  ...
  const entities = useMemo(() => {
    if (!selectedEntityId) return data.entities;
    const filtered = data.entities.filter((e) => e.id === selectedEntityId);
    return filtered.length > 0 ? filtered : data.entities;
  ...
  const directsByEntity = useMemo(() => {
    const map: Record<string, DirectInvestment[]> = {};
    for (const e of data.entities) map[e.id] = [];
    for (const d of directInvestments) {
      if (map[d.entity_id]) map[d.entity_id].push(d);
    }
    return map;
  }, [directInvestments, data.entities]);
  ...
  const confirmReassign = () => {
    if (!reassignFundId || !reassignTarget) return;
    reassignFetcher.submit(
      { intent: "reassign-fund", fund_id: reassignFundId, entity_id: reassignTarget },
      { method: "post", action: "/" }
    );
  };

  useEffect(() => {
    if (reassignFetcher.state === "idle" && reassignFetcher.data?.ok) {
  ...
  const layout = useMemo(() => {
    ...
    for (const ent of entities) {
      const childCount =
        (fundsByEntity[ent.id] || []).length + (directsByEntity[ent.id] || []).length;
      let entityW = NODE.entity.w;
      if (childCount > 0) {
        const childrenW = childCount * (NODE.fund.w + SIBLING_GAP) - SIBLING_GAP;
        entityW = Math.max(entityW, childrenW);
      }
    ...
    entities.forEach((ent, ei) => {
      ...
      const entDirects = directsByEntity[ent.id] || [];
      const childCount = entFunds.length + entDirects.length;
      const fundsW = childCount * (NODE.fund.w + SIBLING_GAP) - SIBLING_GAP;
      let fundStartX = entityCenterX - fundsW / 2;
      const fundY = entityY + NODE.entity.h + LEVEL_GAP;

      const placeDirects = () => {
        entDirects.forEach((di) => {
          const cx = fundStartX + NODE.fund.w / 2;
          nodes.push({
            id: di.id, type: "direct", x: cx, y: fundY,
            w: NODE.fund.w, h: NODE.fund.h, parentId: ent.id, data: di,
          });
          lines.push({ x1: entityCenterX, y1: entityY + NODE.entity.h, x2: cx, y2: fundY });
  ...
  }, [entities, fundsByEntity, directsByEntity, expanded, funds]);
      ...
      <div ref={scrollRef} className="flex-1 overflow-auto relative">
        ...
        >
          ...
          {layout.nodes.map((n) => {
            ...
            if (n.type === "direct") {
              const di = n.data as DirectInvestment;
              const moic = di.cost > 0 ? di.valuation / di.cost : 0;
              const riskColor =
                di.risk_rating === "green"
                  ? "var(--color-atlas-green)"
                  : di.risk_rating === "red"
                    ? "var(--color-atlas-red)"
                    : "var(--color-atlas-orange)";
              const shortName = di.name.length > 16 ? di.name.slice(0, 15) + "…" : di.name;
              return (
                <g key={n.id} className="cursor-pointer"
                  onClick={() => navigate(`/directs/${di.id}`)}
                  onMouseEnter={(e) => setTooltip({ x: e.clientX, y: e.clientY, content: `Direct · ${di.sector} · MOIC ${formatMultiplier(moic)} · NAV ${formatCurrency(di.valuation)}` })}
                  onMouseLeave={() => setTooltip(null)}
                >
                  <rect
                    x={n.x - n.w / 2} y={n.y} width={n.w} height={n.h} rx={2}
                    fill="var(--color-atlas-surface)" stroke={riskColor} strokeWidth="1.5"
                    strokeDasharray="3 2"
                  />
                  <text x={n.x} y={n.y + 16} textAnchor="middle" fill="var(--color-atlas-white)" fontSize="10" fontWeight="600" fontFamily="DM Sans">
                    {"◆ "}{shortName}
                  </text>
                  <text x={n.x} y={n.y + 28} textAnchor="middle" fill="var(--color-atlas-gray3)" fontSize="8" fontFamily="DM Sans">
                    {di.asset_class}
                  </text>
                  <text x={n.x} y={n.y + 40} textAnchor="middle" fill="var(--color-atlas-purple)" fontSize="10" fontWeight="500" fontFamily="IBM Plex Mono">
                    {formatCurrency(di.valuation)}
                  </text>
                </g>
              );
            }
        ...
        {data.entities.length === 0 && (
          <div className="flex justify-center px-6 pb-10">
            <div className="max-w-sm w-full bg-atlas-card border border-atlas-border rounded-xl p-5 text-center">
              <div className="text-[10px] font-bold text-atlas-purple uppercase tracking-widest mb-2">
                {te.emptyLabel}
              </div>
              <div className="text-[14px] font-semibold text-atlas-white mb-1.5">
                {te.emptyTitle}
              </div>
              <div className="text-[12px] text-atlas-gray3 mb-4 leading-relaxed">
                {te.emptyBody}
              </div>
              <button
                onClick={() => navigate("/settings")}
                className="px-4 py-2 rounded-lg bg-atlas-purple text-atlas-white text-[12px] font-semibold cursor-pointer border-none hover:opacity-90 transition-opacity"
              >
                {te.emptyCta}
              </button>
            </div>
